...
- 手动部署和回滚任务
- 固定流水线：拉代码、单元测试、集成测试、构建、部署、健康检查、通知
- 同一项目同一时间只允许一个 pending/running 任务
- 部署日志文件记录和 SSE 实时流
- 应用日志 tail 和 SSE 实时流
- 部署记录、阶段记录、Webhook 事件记录
- 通用 WebHook 通知，兼容常见文本消息格式
...
单元测试命令：cd /data/repos/my-app && mvn clean test
集成测试命令：cd /data/repos/my-app && mvn verify
构建命令：cd /data/repos/my-app && mvn package -DskipTests
部署命令：bash /data/apps/my-app/deploy.sh
...
项目的 `notify_webhook` 支持通用文本消息。部署成功、部署失败、健康检查失败、回滚成功、回滚失败时会尝试通知。

通知失败不会改变部署任务最终状态，但会写入部署日志和后端日志。
...
- MCP 触发部署和回滚默认关闭
- 部署命令有超时控制
- 后端日志不要打印完整 token、secret、notify_webhook
