...
Deploys run the project's ordered `deploy_stages` (`[]model.ProjectStage{Name, Command, Enabled, ContinueOnError}`), then the fixed `health_check → notify` tail; rollbacks stay fixed: `rollback → health_check → notify`. Disabled stages are skipped; a stage with `ContinueOnError` records as failed but does not abort the task. Empty commands are **skipped**, not failed. Projects with no `deploy_stages` fall back to the legacy `pull_code → unit_test → integration_test → build → deploy` order built from the deprecated `*_cmd` fields (`service.LegacyDeployStages`), and `db.Open` backfills those into `deploy_stages` on startup. `pull_code` auto-generates `git fetch --all && git reset --hard origin/<branch>` when `pull_cmd` is empty.
